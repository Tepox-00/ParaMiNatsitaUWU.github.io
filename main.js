...
function setKey(code, val) {
    if(code === 'KeyW' || code === 'ArrowUp') {
        state.keys.w = val;
        if(val) state.keys.s = false; 
    }
    if(code === 'KeyS' || code === 'ArrowDown') {
        state.keys.s = val;
        if(val) state.keys.w = false;
    }
    if(code === 'KeyA' || code === 'ArrowLeft') {
        state.keys.a = val;
        if(val) state.keys.d = false; 
    }
    if(code === 'KeyD' || code === 'ArrowRight') {
        state.keys.d = val;
        if(val) state.keys.a = false;
    }
...
function initSurvivalGame() {
    ...
    const player = { x: canvas.width/2, y: canvas.height/2, w: 32, h: 36, speed: 0.75, hp: 5, dir: {x:0, y:1}, dashCd: 0, facing: 1 };
    const enemies = [];
    const items = [];
    const totalItems = 8;
    ...
    function loop() {
        ...
        ctx.fillStyle = 'white';
        ctx.fillText("NAVE", shipX, shipY - 10);

        if(collectedCount >= 8) {
            const dx = player.x - shipX;
            const dy = player.y - shipY;
        ...
        enemies.forEach((en, idx) => {
            ...
            if(dist < 20) {
                player.hp--;
                document.getElementById('surv-hp').innerText = player.hp;
    ...
    function loop() {
        ...
        enemies.forEach((en, idx) => {
            ...
            if(en.x < player.x + player.w && en.x + en.w > player.x && en.y < player.y + player.h && en.y + en.h > player.y) {
                   player.hp--;
                   document.getElementById('space-hp').innerText = player.hp;
                   enemies.splice(idx, 1);
        ...
        if(progress >= 100) {
            cancelAnimationFrame(spaceLoop);
            startSpaceEndInterlude();
            return;
        }
